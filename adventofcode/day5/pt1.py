...
def follow_the_map(index,map) -> int:
    for row in map:
        if index in range(row[1],row[1]+row[2]):
            new_location = row[0]+index-row[1]
            return new_location
    return index
seeds = []
seed_to_soil_map = []
...
seeds = [0]
for seed in seeds:
    soil = follow_the_map(seed,seed_to_soil_map)
    fertilizer = follow_the_map(soil,soil_to_fertilizer_map)
    water = follow_the_map(fertilizer,fertilizer_to_water_map)
    light = follow_the_map(water, water_to_light_map)
    temp = follow_the_map(light, light_to_temperature_map)
    humid = follow_the_map(temp, temperature_to_humidity_map)
    location  = follow_the_map(humid, humidity_to_location_map)
